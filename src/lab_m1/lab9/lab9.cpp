...
void Lab9::Update(float deltaTimeSeconds)
{
    ...
    //TODO
    {
        isMixed = true;
        glm::mat4 modelmatrix = glm::mat4(1);
        modelmatrix = glm::translate(modelmatrix, glm::vec3(2.5f, 1.8f, -1));
        RenderSimpleMesh(meshes["square"], shaders["LabShader"], modelmatrix, mapTextures["earth"], mapTextures["random"]);
        isMixed = false;
    }
}
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    int loc_is_mixed = glGetUniformLocation(shader->program, "is_mixed");

    if (isMixed) {
        glUniform1i(loc_is_mixed, 1);
    }
    else {
        glUniform1i(loc_is_mixed, 0);
    }

    //glUniform1i(glGetUniformLocation(shader->program, "is_mixed"), is_mixed)
