...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_H)) lightPosition2 -= forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_B)) lightPosition2 -= right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_N)) lightPosition2 += forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_M)) lightPosition2 += right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_J)) lightPosition2 += up * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_G)) lightPosition2 -= up * deltaTime * speed;
    }
