...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int loc_matrial_kd = glGetUniformLocation(shader->program, "material_kd");
    glUniform1f(loc_matrial_kd, materialKd);

    int loc_matrial_ks = glGetUniformLocation(shader->program, "material_ks");
    glUniform1f(loc_matrial_ks, materialKs);
